...
module.exports = function (opts) {
  ...
  return async function bodyParser(ctx, next) {
    if (ctx.location.body !== undefined) return await next();
    if (ctx.location.disableBodyParser) return await next();
    try {
      const res = await parseBody(ctx.req);
      ctx.location.body = 'parsed' in res ? res.parsed : {};
      if (ctx.location.rawBody === undefined) ctx.location.rawBody = res.raw;
    } catch (err) {
      if (onerror) {
        onerror(err, ctx);
      } else {
