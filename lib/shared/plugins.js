  ...
  async install(factory) {
    ...
    for (let i = 0; i < sort.length; i++) {
      ...
      for (const j in plugin.dependencies) {
        const _version = plugin.dependencies[j];
        const version = list[j].version;
        if (compare(version, _version) === -1) throw new Error(`plugin(${j}) need version ${_version}, but got ${version}.You should update this package first.`);
      }
      factory.addPackage(plugin.path, plugin.name);
      this.configs[plugin.name] = pluginConfigs[plugin.name];
      const _plugin = new Pluigin(this.app, this.configs[plugin.name]);
      this.childrens[plugin.name] = _plugin;
    }
    this.childrens._ = new Pluigin(this.app, this.configs._);
  }
