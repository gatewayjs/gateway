...
module.exports = class Processor {
    ...
    if (this.SYS_IS_MASTER) {
      if (this.IS_AGENT) {
        return this.create()
          ...
          .catch(e => process.send({ event: 'ready', error: e.stack }));
      } else {
        return this.create().catch(e => {
          if (e) {
            this.SYS_ERRORS.push(e.stack);
          }
          this.kill();
        });;
      }
  ...
  registerAgent(name, target) {
    ...
    target.on('exit', () => component.status = 2);
  ...
  CLOSE(error) {
    ...
    this.SYS_CLOSING = 1; // 1
    if ((this.SYS_IS_MASTER && !this.SYS_CHILD) || this.SYS_SIGKILL) this.SYS_CLOSING = 2;
    this.debug('prepare closing', this.SYS_CLOSING);
    const timer = setInterval(() => {
      switch (this.SYS_CLOSING) {
        case 2: this.CLOSE_WORKERS(); break;
        case 4: this.CLOSE_AGENTS(); break;
        case 6: this.CLOSING(timer); break;
      }
    }, CUSTOM_TIME);
  }
  ...
  CLOSE_AGENTS() {
    this.SYS_CLOSING = 5;
    this.debug('closing agents', this.SYS_CLOSING);
    const timer = setInterval(() => {
      for (const agent in this.SYS_AGENTS) {
        if (this.SYS_AGENTS[agent].status !== 2) return;    
      }
      clearInterval(timer);
      this.SYS_CLOSING = 6; // 5
      this.debug('closed agents', this.SYS_CLOSING);
    }, CUSTOM_TIME);
    for (const agent in this.SYS_AGENTS) {
      if (this.SYS_AGENTS[agent].status === 0) {
  ...
  CLOSE_WORKERS() {
    this.SYS_CLOSING = 3;
    this.debug('closing workers', this.SYS_CLOSING);
    const timer = setInterval(() => {
      ...
      clearInterval(timer);
      this.SYS_CLOSING = 4; // 3
      this.debug('closed workers', this.SYS_CLOSING);
    }, CUSTOM_TIME);
  ...
  CLOSING(...timer) {
    this.SYS_CLOSING = 7;
    this.debug('closing self', this.SYS_CLOSING);
    if (!this.destroy) {
    ...
    .then(() => {
      this.CLOSE_WITH_MASTER();
      this.debug('closed', this.SYS_CLOSING);
      process.exit(0);
    });
  }
    ...
    if (this.SYS_SIGKILL) {
      ...
      if (this.SYS_IS_MASTER && !this.SYS_CHILD) {
        if (this.SYS_ERRORS.length) {
          console.info(`\n😪  ${chalk.gray('Sorry, there are errors in booting:')}\n`);
          ...
          console.info('\n');
          console.log(chalk.gray(`👀  You can try to solve these problems and restart the startup service again.`));
          console.log('\n');
        }
      }
    }
