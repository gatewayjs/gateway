...
module.exports = class Processor {
  ...
  CLOSE(error) {
    ...
    if (!this.SYS_IS_MASTER) this.SYS_CLOSING = 3;
    this.debug('prepare closing', this.SYS_CLOSING);
    const timer = setInterval(() => {
      switch (this.SYS_CLOSING) {
        case 1: this.CLOSE_WORKERS(); break;
        case 2: this.CLOSE_AGENTS(); break;
        case 3: this.CLOSING(timer); break;
      }
    }, CUSTOM_TIME);
  }
  ...
  CLOSE_AGENTS() {
    ...
    if (!Object.keys(this.SYS_AGENTS).length) {
      this.SYS_CLOSING = 3;
    ...
    const timer = setInterval(() => {
      for (const agent in this.SYS_AGENTS) {
        ...
        clearInterval(timer);
        this.SYS_CLOSING = 3; // 5
        this.debug('closed agents', this.SYS_CLOSING);
      }
    }, CUSTOM_TIME);
    for (const agent in this.SYS_AGENTS) {
      if (this.SYS_AGENTS[agent].status === 2) continue;
